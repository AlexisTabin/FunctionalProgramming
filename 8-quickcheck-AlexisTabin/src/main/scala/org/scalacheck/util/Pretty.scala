...
object Pretty {
  ...
  def pretty[T](t: T, prms: Params)(implicit view: T => Pretty): String = {
    val p = (view(t): Pretty) match {
  ...
  def pretty[T](t: T)(implicit view: T => Pretty): String = pretty(t, defaultParams)

  implicit class strBreak(s1: String) {
    def /(s2: String): String = if(s2 == "") s1 else s1+"\n"+s2
  }
  ...
  implicit def prettyThrowable(e: Throwable): Pretty = Pretty { prms =>
    ...
    val strs2: List[String] =
      if(prms.verbosity <= 0) Nil
