...
object Prop {
  ...
  class ExtendedAny[T](x: => T)(implicit view: T => Pretty) {
    /** See [[Prop.imply]] */
  ...
  implicit def AnyOperators[T](x: => T)(implicit view: T => Pretty): ExtendedAny[T] = new ExtendedAny[T](x)
  ...
  def collect[T, P](f: T => P)(implicit view: P => Prop): T => Prop = t => Prop { prms =>
    val prop = f(t)
    view(prop)(prms).collect(t)
  }
  ...
  def secure[P](p: => P)(implicit view: P => Prop): Prop =
    try view(p) catch { case e: Throwable => exception(e) }
