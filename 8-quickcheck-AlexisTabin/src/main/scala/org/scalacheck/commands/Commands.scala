...
trait Commands {
  ...
  /** A command that runs a sequence of other commands */
  private final class CommandSequence(
    val head: Command, val snd: Command, val rest: Command*
  ...
  private final class Actions(
    val s: State, val seqCmds: Commands, val parCmds: List[Commands]
  ) {
    def copy(s: State = this.s, seqCmds: Commands = this.seqCmds, parCmds: List[Commands] = this.parCmds) = new Actions(s, seqCmds, parCmds)
  }
  ...
  private def actions(threadCount: Int, maxParComb: Int): Gen[Actions] = {
    ...
    } yield new Actions(s0, seqCmds, parCmds)
