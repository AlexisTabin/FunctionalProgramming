...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (SortedMap[Char, Int]() ++ ((w.toLowerCase groupBy((element : Char) => element)) map {case (c, cs) => (c, cs.length)})) toList
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = (occurrences foldRight List[Occurrences](Nil)) {
      case ((c, nbr), acc) => {
        acc ++ (for {comb <- acc; n <- 1 to nbr} yield (c, n) :: comb)
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char,Int]() ++ x){
      case (map, (c, nbr)) => {
             if ((map(c) - nbr) != 0) map updated (c, map(c) - nbr)
             else map - c
      }
    }.toList
  }
