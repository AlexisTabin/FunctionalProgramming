 ...
 */
 class ListsSuite {
  ...
  def average(project : Double, midterm : Double, finale : Double) : Double = {
      (project * 25 + midterm * 35 + finale * 40) / 100
    }

  @Test def `average parCon` : Unit =  {
    val projectGrade : Double = (5*10 + 8)/12 + 1
    val midtermGrade : Double = 21/12 + 1
    val finalGrade = 4.5
    assert(average(projectGrade, midtermGrade, finalGrade) == 6)
    println(average(projectGrade, midtermGrade, finalGrade))
  }
